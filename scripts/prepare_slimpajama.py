...
def parse_args():
    parser = argparse.ArgumentParser(description="Stream and tokenize a bounded SlimPajama subset.")
...
def open_stream(dataset_name: str, dataset_config: str, split: str, seed: int, shuffle_buffer: int):
    try:
        ...
        raise RuntimeError(
            "Required module 'datasets' is missing from the repo venv. Install it with:\n"
            "  .venv/bin/python -m pip install -e '.[train]'\n"
            "or minimally:\n"
            "  .venv/bin/python -m pip install -U datasets zstandard"
        ) from exc
...
def main():
    ...
    print("[prepare] Starting SlimPajama preparation", flush=True)
