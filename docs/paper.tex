...
\subsection{Training Model Shape and Dimensions}

All controlled language-model experiments use the same small decoder-only \texttt{MambaLMHeadModel} shape unless a local-convolution control is explicitly named. The input batch is token IDs of shape $B\times L$, with $L=128$. During training, each micro-batch has $B=8$ and gradient accumulation gives an effective global batch of 64 sequences, or 8192 tokens per optimizer step. The embedding maps tokens to hidden states of shape $B\times L\times160$.

The backbone has 8 residual mixer blocks, RMSNorm with fused add+norm, no attention layers, no MLP branch ($d_{\mathrm{intermediate}}=0$), residuals kept in fp32, and a tied language-model head over the 50,280-token GPT-NeoX vocabulary. For \modelname, each mixer expands from $d_{\mathrm{model}}=160$ to $d_{\mathrm{inner}}=320$. With head dimension 32, this gives 10 recurrent heads. Each head carries a SISO state of size $32\times64$, so the recurrent state tensor per layer has shape $B\times10\times32\times64$. The $B$ and $C$ streams each have state dimension 64 with one group, RoPE is applied to half of the state stream, and chunked training uses chunk size 16. The Simpson and trapezoid \modelname variants differ only in the recurrence coefficients and optional midpoint-control projection; the local-conv variants add a depthwise causal convolution of width 4 over the $x/B/C$ stream.

\begin{table}[t]
\caption{Training model shape used for the controlled language-model runs. Parameter counts include tied embeddings and the LM head.}
\label{tab:model_shape}
\centering
\footnotesize
\begin{tabular}{@{}lr@{}}
\toprule
...
Sequence length & 128\\
Layers & 8\\
Hidden width $d_{\mathrm{model}}$ & 160\\
Expansion / inner width & $2\times$ / 320\\
Head dimension / heads & 32 / 10\\
SSM state dimension & 64\\
Per-layer recurrent state & $B\times10\times32\times64$\\
Training micro/global batch & 8 / 64\\
\modelname{} parameters & 9.51M--9.53M\\
\mambatwo{} baseline parameters & 9.47M\\
\bottomrule
\end{tabular}
\end{table}
...
The first failure mode was exploding or nonfinite gradients before warmup completed. The loss appeared to plateau because clipping forced updates to a bounded norm, but gradient-health logs later showed nonfinite parameters/gradients. Appendix Fig.~\ref{fig:app_initial_instability} and the per-run chart in Fig.~\ref{fig:run-chart-initial-instability-diagnosis-unstable-fp16-lr-1e-4-run} show the representative unstable run: the pre-clip gradient norm remains large while the learning rate is still tiny, and the job terminates at the first nonfinite detection.
...
The validation curves in Appendix Fig.~\ref{fig:app_main_val} are the strongest evidence against the current Simpson formulation. The corresponding per-run charts are Figs.~\ref{fig:run-chart-main-500m-token-comparison-mamba2}--\ref{fig:run-chart-main-500m-token-comparison-simamba-trapezoid}. \mambatwo is the best overall model, but it is not a pure discretization baseline. More importantly, the matched \modelname trapezoid baseline is competitive with and slightly better than default Simpson, while Simpson+midpoint has a better best checkpoint but worse final validation.

\subsection{Matched 50M-Token Discretization Ablation}

To reduce runtime and isolate the discretization, we ran a 50M-token matched ablation with identical model size, data, batch size, seed, and schedule. Table~\ref{tab:ablation50} and Appendix Fig.~\ref{fig:app_ablation50} show that lowering the initial Simpson correction helps, but not enough to beat trapezoid. The individual run charts are Figs.~\ref{fig:run-chart-matched-50m-token-discretization-ablation-trapezoid}--\ref{fig:run-chart-matched-50m-token-discretization-ablation-simpson-low-control}.
...
The benchmark plots are collected in Appendix Figs.~\ref{fig:app_bench_latency}--\ref{fig:app_bench_ratio}. The current prefill gap is a systems bottleneck independent of validation loss.
...
Compression was evaluated as a side experiment using non-destructive weight perturbations on fixed validation spans. Row-wise int8 quantize-dequantize is nearly lossless for all three checkpoints, while int4 and unstructured pruning beyond 10\% cause substantial degradation; Appendix Fig.~\ref{fig:app_compression} shows the corresponding loss deltas.
...
\appendixwidefigure{wandb_main_500m_val_loss.png}{Fixed validation loss on the 500M-token comparison. \mambatwo continues improving through the second pass; Simpson variants begin to degrade on validation after their best checkpoints.}{fig:app_main_val}
...
\appendixwidefigure{wandb_50m_discretization_val_loss.png}{50M-token discretization ablation. The low-control initialization improves Simpson, but the matched trapezoid update remains ahead.}{fig:app_ablation50}

\appendixwidefigure{wandb_localmix_val_loss.png}{Local-mixing controls. Adding Mamba2-style local mixing narrows the gap for Simpson, but local-conv trapezoid is still better at this budget.}{fig:app_localmix}
...
\input{assets/paper/per_run_charts.tex}
